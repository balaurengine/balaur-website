...
[Discussions](https://github.com/balaurengine/balaur/discussions)`,
  ...
  items: {
    ...
    'Semantic tokens, inlay hints and code actions':
      `<>The three LSP surfaces the script tooling left out. Semantic tokens colour a script from what the compiler resolved rather than from a TextMate grammar's guess, so a shadowed local and a module function stop looking alike. Inlay hints show an inferred type or an argument name beside the code that omitted it. A code action offers the fix on a diagnostic: import the module, spell the call the way it is registered. All three wait until completion, hover and rename have been used enough to say which gaps are real. Waiting with them: a completion that reuses the compiled unit instead of building one per keystroke.</>`,
