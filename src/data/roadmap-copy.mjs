...
//
// `shots` is the built half: a picture of the thing and the posts that
// announced it, one entry per item in a `built` milestone. `essays` names the
// posts that are records rather than features — a first commit, a set of
// numbers, a release — so the generator can insist every other post belongs to
// a row.
