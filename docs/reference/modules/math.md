...
sidebar_label: "math"
description: "Deterministic float maths, backed by pure-Rust libm: the same inputs give the same outputs on every platform. A script uses these rather than the language's…"
...
Deterministic float maths, backed by pure-Rust `libm`: the same inputs give the same outputs on every platform. A script uses these rather than the language's own float methods, which reach for the platform's libm and drift between machines.
