...
description: "Asset definitions by reference: a project-relative file path, file#entry for one entry inside it, #id for a block the scene declares, or id://<id> for a…"
...
Asset definitions by reference: a project-relative file path, `file#entry` for one entry inside it, `#id` for a block the scene declares, or `id://<id>` for a path `assets/index.toml` names so the reference survives a rename. A script gets the definition table, not the parsed object the owning plugin builds from it.

11 functions, 0 constants. Scripts reach it as `assets::`.
