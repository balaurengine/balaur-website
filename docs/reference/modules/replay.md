...
description: "Record a running game and play it back. A recording holds each tick's input, network arrivals and events, so a…"
...
Record a running game and play it back. A recording holds each tick's input, network arrivals and events, so a session is small and replays by re-running the game against the same input. The editor's Session dock drives these, and so does `balaur run --record`.
