...
description: "Named looks for the node. Every key beside current and duration is a state holding per-component property tables; node.states.go(\"hover\") patches one…"
...
Methods of `node.states`, the handle a node carrying this component exposes. Every handle also has `get()`, `set(table)`, `patch(table)`, `has()` and `remove()`.
