...
| --- | --- | --- | --- |
| `cap` | enum | `round` | How an open polyline ends One of `round`, `butt`, `square`. |
...
| `gradient_steps` | int | `32` | How many colours a polyline's gradient steps through along its length At least 1. |
...
| `join` | enum | `round` | How a polyline's segments meet One of `round`, `miter`, `bevel`. |
...
| `miter_limit` | float | `4` | How far a miter join may reach, in half-widths, before its corner is cut to a bevel At least 1. |
...
| `segments` | int | `32` | Cuts around a circle, an ellipse, a rounded corner, or a polyline's round joins and caps At least 3. |
| `sides` | int | `4` | Sides, when kind is ngon At least 3. |
| `taper` | vec2 | `[1,1]` | Multipliers on `width` at a polyline's start and end, blended along it; anything but [1, 1] draws round joins and caps |
| `texture` | asset · [`texture`](../assets/texture.md) | — | An image, or a `texture` asset, drawn along a polyline, repeating once per world unit of its length |
...
Methods of `node.shape2d`, the handle a node carrying this component exposes. Every handle also has `get()`, `set(table)`, `patch(table)`, `has()` and `remove()`.
