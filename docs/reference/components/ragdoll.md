...
| --- | --- | --- | --- |
| `blend` | float | `1` | How much of the simulated pose the bones take Range 0–1. |
| `bodies` | node | — | The node holding the bodies, one per bone, named after it |

## Script functions

Methods of `node.ragdoll`, the handle a node carrying this component exposes. Every handle also has `get()`, `set(table)`, `patch(table)`, `has()` and `remove()`.

From [`physics`](../modules/physics.md):
