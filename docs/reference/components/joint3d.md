...
description: "Joins this node's body to body. kind is fixed, revolute, prismatic, spherical, rope, spring or generic; both ends need a body3d on or above the node."
...
In a scene, `joint3d` is the node key that applies it. A script reaches each property below as a field on `node.joint3d`: reading one asks the running component, and assigning one leaves the rest alone. The whole table is `node.joint3d.get()` and `node.joint3d.set(table)`.
...
| `kind` | enum | `fixed` | How the two bodies may move relative to each other One of `fixed`, `revolute`, `prismatic`, `spherical`, `rope`, `spring`, `generic`. |
...
Methods of `node.joint3d`, the handle a node carrying this component exposes. Every handle also has `get()`, `set(table)`, `patch(table)`, `has()` and `remove()`.
