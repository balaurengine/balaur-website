...
| `check_node` | string | — | Node path whose script answers the transitions' `check` methods; empty means this node |
...
| `travel(string)` | Head for the named state through the cheapest chain of transitions, each costing its priority and fading as it says; a state no transition reaches is cut to directly. |
