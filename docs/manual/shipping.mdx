...
A game's first frames have nothing to draw yet. `[application] splash` is the
picture that goes over them, on every target, a browser tab included. It is
project-relative, fits the screen and keeps its aspect.
...
A script that reports a load holds the splash past its seconds, and draws a bar
under the picture. One that reports nothing leaves at its seconds.
...
The seconds are engine time, so a recorded session replays the splash for as
long as the run showed it.
...
- An update follows the channel its own version names: `0.2.0-alpha.3` tracks
  the alpha line, a plain version tracks stable, and a nightly follows the
  rolling nightly. A source build refuses and points at git.
- `--check` only reports. `--channel alpha` crosses to another line, `--tag`
  picks one exact release, and `--allow-downgrade` takes a step backwards.
