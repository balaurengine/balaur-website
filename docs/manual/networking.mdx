...
The first is a handler method: a request or connection names the node that
handles its results, dispatched as method calls.
...
The second is a sequential await: a request without a node returns a token
the script suspends on, from an `async` entry point.
...
## Rollback, and what is missing

Rollback is built. Two engines exchange only inputs, predict each other, roll
...
Replication, RPC, a browser WebTransport backend and WebRTC are missing. The plan is
