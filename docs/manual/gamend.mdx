...
The addon's `client.rn` covers the ways a player gets an account.

```rune
let client = script::require("addons/gamend/client.rn");
task::wait((client.login_device)((), engine::device_id())).await;
task::wait((client.register_email)((), "ana@example.com", "a long password", ())).await;
task::wait((client.login_email)((), "ana@example.com", "a long password")).await;
```
...
- `api.user_delete_current_user(node, params)` deletes the signed-in account.
  An account with a password sends `#{ "current_password": ... }`.
