...
everything runs headless — tests, servers, CI, and `run --headless`.
...
Useful dev flags: `--frames N` stops after N frames, `--screenshot out.png`
saves the window's framebuffer to a PNG, `--headless` runs without a window.
