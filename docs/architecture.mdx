...
- A node is an entity carrying `Name`, `Parent`, `Children` and
  `GlobalTransform`. Its local `Transform` is the `transform` component, and
  a node without one sits where its parent is.
